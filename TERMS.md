...
- 끝나는 조건이 있는 프로젝트는 `완료형`, 계속 업무가 들어오는 프로젝트는 `운영형`으로 표현해요.
- 완료형은 전체 실행할 일 중 완료한 일의 비율을 `진행률`로 보여줘요.
- 운영형에는 진행률을 쓰지 않고 새로 들어온 일, 완료한 일, 열린 일 변화, 정체된 일을 보여줘요.
- `열린 일 변화`는 한 주 시작 시점보다 현재 남은 일이 얼마나 늘거나 줄었는지 중립적으로 보여줘요.
- 새로 추가한 일은 `밀린 일`이나 `지연`으로 표현하지 않아요.
- 실제 지연은 `기한 지난 일`, 오랫동안 바뀌지 않은 일은 `정체된 일`로 구분해요.
- `정체된 일`은 프로젝트에서 정한 기간 동안 바뀌지 않은 열린 일이에요.
- 주간 리포트는 월요일부터 현재까지의 실제 기록을 정리하며, 근거 없는 성과 점수나 달성 확률을 만들지 않아요.

### 외부 자동화 연결

- 사용자가 직접 설정하는 프로젝트 자동화 기능에는 `웹훅`을 사용해요.
- URL, payload, endpoint 같은 하위 구현 용어는 공개 문구에 노출하지 않아요.
- 회사의 업무 요청 원문 시스템은 사용자가 알고 있는 서비스명인 `ITSM`으로 표시해요.
- ITSM 인증키, 서버 주소, 요청 헤더 같은 연결 내부 정보는 공개 문구에 노출하지 않아요.
