...
use jimin_storage::{
    ...
    work::{
        DeleteProjectOutcome, NewProject, ProjectManagementMode, ProjectStatus, ProjectUpdate,
        WorkspaceScope,
    },
};
...
async fn project_update_is_scoped_versioned_and_emits_current_state() {
    ...
    let updated = database
        .update_project(&ProjectUpdate {
            ...
            management_mode: Some(ProjectManagementMode::Operation),
            reporting_enabled: Some(false),
            stale_threshold_days: Some(14),
            risk_level: 2,
    ...
    assert_eq!(updated.management_mode, ProjectManagementMode::Operation);
    assert!(!updated.reporting_enabled);
    assert_eq!(updated.stale_threshold_days, 14);
    assert_eq!(updated.weekly_created_task_count, 0);
